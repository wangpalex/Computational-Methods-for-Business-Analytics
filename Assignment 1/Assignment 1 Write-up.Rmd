...
# y = 0.734 + 1.954 * x
```
Using lm(), the beta 0 estimate is 0.73407 and the beta 1 estimate is 1.95394.
...
```
Using matrix algebra, the beta 0 estimate is 0.73407 and the beta 1 estimate is 1.95394.
...
```
Using R's optim(), the beta 0 estimate is 0.73406 and the beta 1 estimate is 1.95382.
...
beta1 # 1.954
```
Using formula for 1 variable regression, the beta 0 estimate is 0.73407 and the beta 1 estimate is 1.95394.

Based on the values of estimates from all four methods, all four methods give the same estimates.
...
```
According to the calculations and plots above, res1(with N=25) has a mean of 2.002271 and res2(with N=100) has a mean of 1.998513, also res1 has a variance of 0.04465469 while res2 has a variance of 0.01046778. We can conclude that mean values and variance values differ at around 1e-03 level. Moreover, by running two-sided t-test for the two distribution: res1 and res2, we cannot reject null hypothesis from p-value=0.1095 that res1 and res2 are not statistically different. By running Anderson-Darling normality test, we can reject Anderson-Darling test null hypothesis with p-value=2.064e-08 which indicates non-normal distribution for both res1 and res2.
...
Regarding the variance, res1 has a variance of 0.003772373 while res2 has a variance of 0.0008630136. By comparing the varince of two results which differs at about 1e-03 level and differs by 0.002909359, we may conclude that the variance is different. Further, through F.test (assuming the data is normally distributed), we get p-value < 2.2e-16, thus we can reject the null hypothesis that the variance of two groups are statistically equal i.e. the variance of res1 and res 2 are statistically different. Also, we can conclude that the estimated beta 1 with greater sample size N=100 has a lower variance that the estimated beta 1 with smaller sample size N=25. A larger sample size gives a less spread-out result i.e. data with smaller dispersion. The answer in II does not change for variance coparison.

By observing the QQ-plot, there are only a few points near two ends falling away from the straight line.By running Anderson-Darling normality test, the p-value for both cases = 0.4207, thus we cannot reject Anderson-Darling test null hypothesis that it follows normal distribution and we conclude that both the estimated beta 1 in res1 and res2 follows normal distribution. The answer in II changes for normality.
...
Regarding the variance, res1 has a variance of 3427.213 while res2 has a variance of 29851.75. By comparing the varince of two results which differs much, we may conclude that the variance is different. Further, through F.test (assuming the data is normally distributed), we get p-value < 2.2e-16, thus we can reject the null hypothesis that the variance of two groups are statistically equal i.e. the variance of res1 and res 2 are statistically different. Also, we can conclude that the estimated beta 1 with greater sample size N=100 has a lower variance that the estimated beta 1 with smaller sample size N=25. A larger sample size gives a less spread-out result i.e. data with smaller dispersion. The answer in II does not change for variance coparison.

In addtiona, the difference between two mean values and 2 variance values are bigger when using Cauchy distribution than the previous 2 methods.

By observing the QQ-plot, many points near two ends fall away from the straight line. Also, by running Anderson-Darling normality test, the p-value for both cases = 2.2e-16 < 0.05, thus we should reject Anderson-Darling test null hypothesis at 5% significance level and conclude that both the estimated beta 1 in res1 and res2 follows non-normal distribution. The answer in II does not change for normality.
...
sol1 = newton_method(func2, 0.1)
sol2 = newton_method(func2, 0.2)
sol3 = bisection_method(func2, 0.01, 0.1)
sol4 = bisection_method(func2, 0.01, 0.2)
c(sol1$`optim r`, sol2$`optim r`, sol3$`optim r`, sol4$`optim r`)
```
Bisection method and Newton method come to the same result: monthly interest rate r = 0.01439478.

```{r Q4.3} 

plot(1:length(sol2$history),sol2$history,type="b",pch=20,col="blue",ylim=c(0.01,0.2),xlab="iterations",ylab="r")
